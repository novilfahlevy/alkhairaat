...
@section('content')
    ...
    <div class="rounded-lg bg-white p-6 shadow-md dark:bg-gray-900">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div class="mb-4 md:mb-0">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        Perbarui informasi lembaga {{ $lembaga->nama }}
                    </p>
                </div>
        ...
        <!-- Form -->
        <form action="{{ route('lembaga.update', $lembaga) }}" method="POST" id="lembagaForm">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="kode_lembaga" value="{{ old('kode_lembaga', $lembaga->kode_lembaga) }}" 
                    ...
                    <div>
                        ...
                        <input type="text" name="nama" value="{{ old('nama', $lembaga->nama) }}" 
                    ...
                    <div>
                        ...
                        <select name="jenjang" 
                            ...
                            @foreach($jenjangOptions as $jenjang)
                                <option value="{{ $jenjang }}" {{ old('jenjang', $lembaga->jenjang) == $jenjang ? 'selected' : '' }}>
                                    {{ $jenjang }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="status" 
                            ...
                            @foreach($statusOptions as $status => $label)
                                <option value="{{ $status }}" {{ old('status', $lembaga->status) == $status ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="provinsi_id" id="provinsi" 
                            ...
                            @foreach($provinsi as $prov)
                                <option value="{{ $prov->id }}" {{ old('provinsi_id', $lembaga->kabupaten?->provinsi_id) == $prov->id ? 'selected' : '' }}>
                                    {{ $prov->nama_provinsi }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="kabupaten_id" id="kabupaten"
                                class="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 @error('kabupaten_id') border-red-500 @enderror">
                            <option value="">Pilih Kabupaten/Kota</option>
                            @foreach($kabupaten as $kab)
                                <option value="{{ $kab->id }}" {{ old('kabupaten_id', $lembaga->kabupaten_id) == $kab->id ? 'selected' : '' }}>
                                    {{ $kab->nama_kabupaten }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="alamat" rows="3" placeholder="Alamat lengkap lembaga"
                                class="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 @error('alamat') border-red-500 @enderror">{{ old('alamat', $lembaga->alamat) }}</textarea>
                        @error('alamat')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            fetch(`{{ route('lembaga.get_kabupaten') }}?provinsi_id=${provinsiId}`)
                ...
                .then(data => {
                    data.forEach(kabupaten => {
                        const option = new Option(kabupaten.nama_kabupaten, kabupaten.id);
