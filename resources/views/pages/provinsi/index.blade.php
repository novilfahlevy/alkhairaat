                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
                                <span class="inline-flex rounded-full bg-blue-100 px-2 py-1 text-xs font-medium text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
                                    {{ $item->kabupaten->count() }} kabupaten
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('provinsi.destroy', $item) }}" method="POST" class="inline"
            ...
            <div class="space-y-4">
                @forelse($provinsi as $item)
                    <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        ...
                        @canany(['manage_provinsi', 'access_provinsi'])
                            <div class="flex items-center gap-2">
                                <a href="{{ route('provinsi.show', $item) }}"
                                ...
                                <form action="{{ route('provinsi.destroy', $item) }}" method="POST" class="flex-1"
                                    onsubmit="return confirm('Apakah Anda yakin ingin menghapus provinsi ini?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
