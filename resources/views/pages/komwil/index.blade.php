    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">
                Kelola akun Komisariat Wilayah (Komwil)
            </p>
        </div>
        <div>
            <a href="{{ route('manajemen.komwil.create') }}"
                    ...
                    @forelse ($users as $user)
                        <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td class="px-4 py-3 text-center">
                                <a href="{{ route('manajemen.komwil.edit', $user->id) }}" class="text-brand-500 hover:underline">Edit</a>
                                <form action="{{ route('manajemen.komwil.destroy', $user->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-500 hover:underline ml-2" onclick="return confirm('Yakin ingin menghapus Komwil ini?')">Hapus</button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr class="border-b border-gray-200 dark:border-gray-700">
                            <td colspan="4" class="px-4 py-3 text-center text-sm text-gray-500 dark:text-gray-400">Belum ada Komwil</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        @if ($users->hasPages())
            <div class="mt-6 flex items-center justify-between">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Menampilkan {{ $users->firstItem() }} hingga {{ $users->lastItem() }} dari {{ $users->total() }} Komwil
                </div>
                <div>
