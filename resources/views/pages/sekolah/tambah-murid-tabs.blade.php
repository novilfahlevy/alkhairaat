    ...
    <div class="border-b border-gray-200 dark:border-gray-700">
        <div class="flex overflow-x-auto whitespace-nowrap scrollbar-hide" role="tablist">
            <a href="{{ route('sekolah.create-murid', ['sekolah' => $sekolah, 'tab' => 'manual']) }}" role="tab"
                ...
                class="inline-block min-w-[180px] border-b-2 px-6 py-4 text-sm font-medium transition {{ $currentTab === 'manual' ? 'border-brand-500 text-brand-600 dark:text-brand-400' : 'border-transparent text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white' }}">
                <i class="fas fa-user-plus mr-2"></i>
                Tambah Murid Manual
            </a>
            <a href="{{ route('sekolah.create-murid', ['sekolah' => $sekolah, 'tab' => 'existing']) }}" role="tab"
                aria-selected="{{ $currentTab === 'existing' ? 'true' : 'false' }}"
                class="inline-block min-w-[180px] border-b-2 px-6 py-4 text-sm font-medium transition {{ $currentTab === 'existing' ? 'border-brand-500 text-brand-600 dark:text-brand-400' : 'border-transparent text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white' }}">
                <i class="fas fa-users mr-2"></i>
                Pilih Murid yang Ada
            </a>
