<x-ui.card class="bg-white dark:bg-gray-900 shadow-lg dark:shadow-gray-800/50">
    ...
    <div class="space-y-4">
        @forelse($provinsis as $provinsi)
            <div x-data="{ open: {{ $loop->first ? 'true' : 'false' }} }" class="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden shadow-sm dark:shadow-gray-800/30">
                ...
                <button type="button" @click="open = !open"
                    ...
                    <div class="flex items-center gap-3 flex-1">
                        ...
                        <svg x-show="open" class="w-5 h-5 text-blue-600 dark:text-blue-300 transition-transform duration-200 flex-shrink-0"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7">
                            </path>
                        </svg>
                        <svg x-show="!open"
                            class="w-5 h-5 text-blue-600 dark:text-blue-300 transition-transform duration-200 flex-shrink-0" fill="none"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        @forelse($provinsi['kabupatens'] as $kabupaten)
                            <div
                                ...
                                <p class="text-sm font-medium text-gray-600 dark:text-gray-300 mb-3">{{ $kabupaten['nama'] }}</p>
                                <div class="space-y-2">
                        ...
                        @empty
                            <p class="text-sm text-gray-500 dark:text-gray-400 col-span-full py-4 text-center bg-gray-50 dark:bg-gray-700 rounded-md">Tidak ada kabupaten</p>
                        @endforelse
                    </div>
                </div>
            </div>
